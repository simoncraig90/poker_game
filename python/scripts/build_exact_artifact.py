...
# ── 4BP family: 2-action [check, jam] for low-SPR play ─────────────────────

FOURBP_ACTIONS = [("check", "none"), ("jam", "none")]

# OOP (hero 4-bet, acting first postflop)
# At SPR < 2, OOP 4-bettor: jam with strong value + some bluffs, check/fold rest.
FOURBP_OOP_MATRIX = [
...
def build_from_manifest(manifest_path, output_dir):
    ...
    4bp keys automatically use the 2-action [check, jam] menu.
    """
    count = 0
    skipped = 0
    with open(manifest_path) as f:
        for line_no, line in enumerate(f, 1):
            ...
            is_4bp = key.startswith("4bp/")
            ip = hero_is_ip(agg, hero)

            if is_4bp:
                actions = FOURBP_ACTIONS
                matrix = FOURBP_IP_MATRIX if ip else FOURBP_OOP_MATRIX
                tag = "4BP-IP" if ip else "4BP-OOP"
            else:
                actions = POSTFLOP_ACTIONS
                matrix = IP_MATRIX if ip else OOP_MATRIX
                tag = "IP" if ip else "OOP"

            dest = write_artifact(output_dir, key, actions, matrix)
            count += 1
            print(f"  [{count:3d}] {tag:7s}  {key}")
